...
class High extends React.Component{
    ...
    changeArea(e){
        ...
        this.props.submitSearch(1,'USA','Area',e.target.innerHTML);
    }
    changeSchoolNature(e){
        this.setState({
            SchoolNature:e.target.innerHTML
        });
        this.props.submitSearch(1,'USA','SchoolNature',e.target.innerHTML);
    }
    changeSchoolCharacteristic(e){
        this.setState({
            SchoolCharacteristic:e.target.innerHTML
        });
        this.props.submitSearch(1,'USA','SchoolCharacteristic',e.target.innerHTML);
    }    
    changeSchoolDormitoryType(e){
        this.setState({
            SchoolDormitoryType:e.target.innerHTML
        });
        this.props.submitSearch(1,'USA','SchoolDormitoryType',e.target.innerHTML);
    }
    changeTuition(e){
        this.setState({
            Tuition:e.target.innerHTML
        });
        this.props.submitSearch(1,'USA','Tuition',e.target.innerHTML);
    }
    changeUniproperty(e){
        this.setState({
            Uniproperty:e.target.innerHTML
        });
        this.props.submitSearch(1,'USA','Uniproperty',e.target.innerHTML);
    }
    changeMoreFilter(){
        this.setState({
